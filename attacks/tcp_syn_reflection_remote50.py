...
class TcpSynReflectionRemote50(GenericAttack):

    def __init__(self, attackName, attackConfig, deviceConfig):
        super(TcpSynReflectionRemote50, self).__init__(attackName, attackConfig, deviceConfig)
	...
	if deviceMode:
        ...
        while self.running:
            if not self.running:
                break
            pkt = IP(src=server_ip, dst=target)
